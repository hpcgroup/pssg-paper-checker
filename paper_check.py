...
SUPPORTED_MODELS = {
    ...
    "GLM-4.7": {"provider": "zai", "vision_model": None},
}


class ChecklistEvaluator:
    def __init__(
        self, pdf_path, latex_path_or_dir, api_key=None, model_name=None, skip_llm=False
    ):
    ...
    def _get_api_key(self):
        ...
        if self.provider == "zai":
            return self.api_key or os.getenv("ZAI_API_KEY")
        return None
    ...
    def load_pdf(self):
        ...
        if (
            not self.pdf_path
            or self.pdf_path == "__NO_PDF__"
            or not os.path.exists(self.pdf_path)
        ):
            return ""
    ...
    def load_latex_dir(self, directory):
        ...
        print(
            f"LaTeX processing complete, merged text length: {len(result)} characters"
        )
    ...
    def extract_sections(self, latex_text):
        ...
        abstract_match = re.search(
            r"\\begin\{abstract\}(.*?)\\end\{abstract\}", latex_text, re.DOTALL
        )
        if abstract_match:
    ...
    def check_section_titles_descriptive(self):
        ...
        title_matches = re.findall(
            r"\\(section|subsection|subsubsection)\{(.+?)\}", self.latex_text
        )
        ...
        result = {
            ...
            "detail": issues
            if issues
            else "All section titles are sufficiently descriptive or are allowed single-word titles.",
    ...
    def check_section_titles_capitalization(self):
        ...
        titles = [
            match[1] for match in title_matches if not match[1].strip().startswith("\\")
        ]
            ...
            for i, word in enumerate(words):
                ...
                if i == 0:
                    if not word_clean[0].isupper():
                        ...
                        errors.append(
                            f"'{word}' should be capitalized as '{correct_word}'"
                        )
        ...
        def is_title_case(s):
            ...
            if ":" in s:
                ...
                for i, part in enumerate(parts):
                    ...
                    part = part.strip()
                    if part and not is_title_part_case(
                        part, True
                    ):  # First word of each part should be capitalized
                        return False
                return True
            else:
        ...
        def is_title_part_case(s, is_first_part=True):
            ...
            for i, word in enumerate(words):
                ...
                if i == 0:
                    if not word_clean[0].isupper():
                        incorrect_case_words.append(
                            (word, True)
                        )  # True means should be capitalized
                # Last word should also be capitalized, but only if it's the first part or the entire title
                elif i == len(words) - 1 and is_first_part:
                    if not word_clean[0].isupper():
                        incorrect_case_words.append(
                ...
                # Check words that should be lowercase but are capitalized
                    ...
                    incorrect_case_words.append(
                        (word, False)
                    )  # False means should be lowercase
        ...
        is_consistent = (title_case_count == len(titles)) or (
            lowercase_count == len(titles)
        )
        ...
        if is_consistent and not title_errors:
            ...
            if title_case_count > lowercase_count:
                ...
                inconsistent_titles = [
                    title for title in titles if not is_title_case(title)
                ]
                preferred_style = "Title Case"
            else:
                # Most are lowercase, find those that don't conform
                inconsistent_titles = [
                    title for title in titles if title.lower() != title
                ]
            ...
            if title_errors:
                ...
                detail = (
                    f"Section titles have capitalization issues. Recommended style: {preferred_style}.\nDetailed errors: "
                    + "\n".join(error_details)
                )
            else:
    ...
    def check_section_buffer(self):
        ...
        for i in range(len(section_matches) - 1):
            ...
            if (current_level == "section" and next_level == "subsection") or (
                ...
                section_start_match = re.search(
                    r"\{" + re.escape(current_title) + r"\}",
                    self.latex_text[current_pos:],
                )
                if section_start_match:
                    ...
                    between_text_no_labels = re.sub(
                        r"\\label\{[^}]*\}", "", between_text
                    ).strip()
        ...
        result = {
            ...
            "detail": issues
            if issues
            else "Sections and subsections have appropriate buffer content.",
    ...
    def _static_check_figures_in_content(self, rel_path, content, full_text):
        ...
        figures_with_options = re.findall(
            r"\\begin\{figure\}\[([^\]]+)\](.*?)\\end\{figure\}",
            non_commented_text,
            re.DOTALL,
        )
        figures_without_options = re.findall(
            r"\\begin\{figure\}(?!\[)(.*?)\\end\{figure\}",
            non_commented_text,
            re.DOTALL,
        )
        ...
        if DEBUG:
            print(
                f"Found {len(figures_with_options)} figures with options and {len(figures_without_options)} figures without options in {rel_path}"
            )
        ...
        for placement, content_block in figures_with_options:
            ...
            caption_preview_short = preview_text(
                caption_text or "Figure without caption", 30
            )

            if "h" not in placement:
                record(
                    f"Figure missing [h] placement option: '{caption_preview_short}'"
                )
            ...
            if label_match:
                ...
                if caption_match and label_match.start() < caption_match.start():
                    record(
                        f"Figure label '{label_text}' should be placed after the caption"
                    )
                if not caption_match:
                    record(
                        f"Figure label '{label_text}' exists but has no associated caption"
                    )
            elif caption_match:
                record(
                    f"Figure has caption but missing label: '{caption_preview_short}'"
                )
    ...
    def static_check_figures(self):
        ...
        for rel_path, content in self._iterate_latex_files():
            file_issues, file_failed_messages = self._static_check_figures_in_content(
                rel_path, content, full_text
            )
    ...
    def static_check_numbers_spelling(self):
        ...
        for rel_path, content in self._iterate_latex_files():
            ...
            issues.append(
                f"{rel_path}: Found standalone numeric digits: {', '.join(digits_in_file)}. Consider spelling them out."
            )
            ...
            for match in matches:
                ...
                failed_messages.append(
                    f"{rel_path}: Number '{match.group(0)}' appears in: '...{context}...'"
                )
        ...
        result = {
            ...
            "detail": issues
            if issues
            else "All numbers are properly spelled out or not standalone.",
            "failed_content": failed_messages if failed_messages else None,
        }
    ...
    def static_check_informal_words(self):
        ...
        informal_words = {"see": "observe", "show": "demonstrate", "like": "such as"}
        issues = []
        failed_messages = []
        ...
        for rel_path, content in self._iterate_latex_files():
            for word, suggestion in informal_words.items():
                matches = list(
                    re.finditer(r"\b" + re.escape(word) + r"\b", content, re.IGNORECASE)
                )
                if not matches:
                    continue

                issues.append(
                    f"{rel_path}: Found informal word '{word}'. Consider replacing with '{suggestion}'."
                )
                ...
                for match in matches:
                    ...
                    context = content[start:end]
                    failed_messages.append(
                        f"{rel_path}: Found informal word '{word}' in context '...{context}...'"
                    )
    ...
    def static_spell_check(self):
        ...
        for m in matches:
            ...
            context = text[
                max(0, m.offset - 30) : min(len(text), m.offset + m.errorLength + 30)
            ]
    ...
    def call_llm(self, prompt, max_tokens=2000):
        ...
        try:
            if self.provider == "xai":
                ...
                if not api_key:
                    raise ValueError(
                        "XAI_API_KEY is not set. Provide it via environment or --api_key."
                    )
                if "xai_text" not in self._clients:
                    self._clients["xai_text"] = OpenAI(
            ...
            if self.provider == "gemini":
                ...
                if not api_key:
                    raise ValueError(
                        "Gemini API key is not set. Provide it via environment or --api_key."
                    )
                if "gemini_text" not in self._clients:
                    genai.configure(api_key=api_key)
                    self._clients["gemini_text"] = genai.GenerativeModel(
                        self.model_name
                    )
            ...
            if self.provider == "openai":
                ...
                if not api_key:
                    raise ValueError(
                        "OpenAI API key is not set. Provide it via environment or --api_key."
                    )
            ...
            if self.provider == "zai":
                ...
                if not api_key:
                    raise ValueError(
                        "ZAI_API_KEY is not set. Provide it via environment or --api_key."
                    )
...
{section_content}"""
        ...
        try:
            if self.provider == "xai":
                ...
                if "xai_vision" not in self._clients:
                    self._clients["xai_vision"] = OpenAI(
                        api_key=api_key, base_url="https://api.x.ai/v1"
                    )
            ...
            if self.provider == "zai":
                return "Vision not supported for zai provider."
    ...
    def llm_evaluate_figures_from_pdf(self):
        ...
        if (
            ...
            return {"error": "No PDF provided, skipping figure vision analysis."}
...
8. Figures should appear right after (using [h]) where they are mentioned (in most cases except layout issues)"""
                ...
                try:
                    ...
                    results[f"page_{i + 1}"] = self.call_vision_llm(
                        prompt, base64_image
                    )
    ...
    def run_all_checks(self):
        ...
        if not self.skip_llm:
            ...
            for section_title, section_content in self.sections.items():
                result = self.llm_evaluate_writing_for_section(
            ...
            self.report["LLM_Writing"] = {
                section_title: skip_message for section_title in self.sections
            }
...
def main():
    parser = argparse.ArgumentParser(
        description="Check paper drafts against quality checklist using LLM and static methods."
    )
    parser.add_argument(
        "--pdf",
        required=False,
        default=None,
        help="Path to the paper draft PDF file (optional, for figure vision analysis)",
    )
    parser.add_argument(
        "--latex", required=True, help="Path to LaTeX source file or directory"
    )
    parser.add_argument(
        "--model",
        required=False,
    ...
    parser.add_argument(
        "--fail-on-error",
        action="store_true",
        help="Exit with code 1 if any check fails (static or LLM)",
    )
    args = parser.parse_args()

    # Use a placeholder if no PDF is provided (similar to static-check mode)
    pdf_path = args.pdf if args.pdf else "__NO_PDF__"
    ...
    if not model_name:
        choice = (
            input("No --model specified. Do you want to skip LLM checks? (y/N): ")
            .strip()
            .lower()
        )
        if choice in ("y", "yes"):
            ...
            while not model_name:
                candidate = input(
                    f"Please specify the model name ({available_models}): "
                ).strip()
                if candidate in SUPPORTED_MODELS:
                    model_name = candidate
                else:
                    print(
                        f"Invalid model name '{candidate}'. Supported models: {available_models}"
                    )
    else:
        if model_name not in SUPPORTED_MODELS:
            raise ValueError(
                f"Unsupported model '{model_name}'. Supported models: {', '.join(SUPPORTED_MODELS.keys())}"
            )
    ...
    if static_checks:
        ...
        passed_checks = sum(
            1 for _, data in static_checks if data.get("result") is True
        )
    ...
    for check_name, check_data in report.items():
        if (
            ...
            and check_data["result"] is False
        ):
    ...
    # Check for failures and exit if --fail-on-error is set
    if args.fail_on_error:
        ...
        for check_name, check_data in report.items():
            if isinstance(check_data, dict) and check_data.get("result") is False:
                has_failures = True
                break
            # Also check for LLM errors
            if isinstance(check_data, dict) and "error" in check_data:
                has_failures = True
                break
        if has_failures:
            raise SystemExit(1)
